...
namespace SPUnit {
    ...
    public:
        ...
        static void run(Scenario& scenario, uint32_t line, TParameterType actual, UParameterType expected, bool invertLogic) {
            ...
            ExpectScenario::fail(scenario, ExpectFailureMessage::makeNoGreaterThanOperator(), line);
        }
    };
    ...
    public:
        ...
        static void run(Scenario& scenario, uint32_t line, TParameterType actual, UParameterType expected, bool invertLogic) {
            if ((!(actual > expected)) != invertLogic) { //!= acts as logical XOR
                ExpectScenario::fail(scenario, ExpectFailureMessage::makeGreaterThan(actual, expected, invertLogic), line);
            }
        }
    };
}