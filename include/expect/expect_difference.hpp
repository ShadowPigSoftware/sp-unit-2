...
namespace SPUnit {
    ...
    template <class T, class U, class Enabler = void> class ExpectDifference {
    public:
    ...
    template <class T, class U> class ExpectDifference<T, U, typename std::enable_if<ExpectDifferenceTest::supported<T,U>::value>::type> {
    public:
        using TParameterType = typename ExpectType<T>::ParameterType;
        using UParameterType = typename ExpectType<U>::ParameterType;
        static void run(Scenario& scenario, uint32_t line, TParameterType actual, UParameterType expected) {
